...
RasterScreen::RasterScreen(int w, int h) : width(trim_width(w)), height(trim_height(h)) {
	pixels = std::vector<sf::RectangleShape>(width * height);
	z_buffer = std::vector<double>(width * height, -1);
